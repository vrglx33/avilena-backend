...
const resolvers = {
    Query: {
        ...
        getAllPendingOrders: async (parent, {user}, {context}) => {
            return await context.prisma.orders(
                {
                    where: {
        ...
        getAllDeliveredOrders: async (parent, {user}, {context}) => {
            return await context.prisma.orders(
                {
                    where: {
                        status: "Entregada"
        ...
        getAllOnWayOrders: async (parent, {user}, {context}) => {
            return await context.prisma.orders(
                {
                    where: {
                        status: "En camino"
    ...
    Order: {
        async user(parent,args,{context}) {
            return await context.prisma.order({ id: parent.id }).user();
        },
    },
    User: {
        async city(parent,args,{context}) {
            return await context.prisma.user({ id: parent.id }).city();
        },
    },
};
